...
hook.Add("Think", "igwars_zone", function()
    ...
    for _, ply in player.Iterator() do
        if ply:Alive() and ply:GetPos():DistToSqr(zonepoint) > radiussqr then
            ply:TakeDamage(2, game.GetWorld(), game.GetWorld())
        end
    end
end)
